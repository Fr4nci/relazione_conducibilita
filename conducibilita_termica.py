...
def safe_extraction(lst, arr1, arr2):
    ...
    for files in sorted(os.listdir(lst)):
        ...
        for num in range(len(data["Temp_0"])-n, len(data["Temp_0"])):
            ...
            # if files == "mr1.txt":
            #    print(str(float(data["Temp_0"][num])) + "\t" + str(float(data["Temp_1"][num])) + "\t" + str(float(data["Temp_0"][num] - data["Temp_1"][num])))
        #m = acc/float(n)
        for num in range(len(data["Temp_0"])-n, len(data["Temp_0"])):
...
plt.ylabel(r"$T_i [C]$")
plt.savefig("Grafico_differenza_temperatura_rame.pdf")
plt.show() # 8 e 12
...
plt.savefig("Grafico_residui_rame.pdf")
plt.show()
S = np.pi * (2.5 * 10 ** (-2)) ** 2
V = 10.2
sigma_V = 0.1
